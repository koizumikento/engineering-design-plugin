...
不足データや未対応項目を `PASS` に変換しない。下面の公称隙間は `ボス高 - 下面最大部品高` で評価する。下面部品がない場合も高さ0を明記し、空欄を0と扱わない。STEP/BREP/PCB 3Dデータがある場合は `references/geometry-checks.md` に従って共通座標系で追加確認する。
